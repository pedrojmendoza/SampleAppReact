  ...
  stages {
    ...
    stage ('UI Test') {
      steps {
        ...
        sh "docker exec cypress sh -c 'cd /my-app && pwd && ls -la && npm config set proxy ${env.HTTP_PROXY} && npm config set https-proxy ${env.HTTPS_PROXY} && npm install cypress --save-dev && ./node_modules/.bin/cypress run'"
